          ...
          if (direction > 0 && newIndex > 199) {
            ...
            setDots((prevDots) => {
              ...
              if (!prevDots.some(dot => dot.position[0] === disappearingArc.olng && dot.position[1] === disappearingArc.olat)) {
                newDots.push({
                  position: [disappearingArc.olng, disappearingArc.olat],
                  id: originKey,
                  code: disappearingArc.meta?.o || ''
                });
              }
              if (!prevDots.some(dot => dot.position[0] === disappearingArc.dlng && dot.position[1] === disappearingArc.dlat)) {
                newDots.push({
                  position: [disappearingArc.dlng, disappearingArc.dlat],
                  id: destKey,
                  code: disappearingArc.meta?.d || ''
                });
              }
              return newDots;
            });
          ...
          if (oldestArcIndex >= 0 && oldestArcIndex < allFlights.length) {
            ...
            setDots((prevDots) => {
              ...
              if (!prevDots.some(dot => dot.position[0] === removingArc.olng && dot.position[1] === removingArc.olat)) {
                newDots.push({
                  position: [removingArc.olng, removingArc.olat],
                  id: originKey,
                  code: removingArc.meta?.o || ''
                });
              }
              if (!prevDots.some(dot => dot.position[0] === removingArc.dlng && dot.position[1] === removingArc.dlat)) {
                newDots.push({
                  position: [removingArc.dlng, removingArc.dlat],
                  id: destKey,
                  code: removingArc.meta?.d || ''
                });
              }
              return newDots;
            });
  ...
  return (
    <div className="relative w-full h-[90vh] bg-black">
      ...
      {hoveredAirport && hoveredAirport.code && (
        <div
          className="absolute pointer-events-none z-30 text-white px-3 py-2 text-lg font-medium"
          style={{
      ...
      <div className="absolute bottom-8 left-24 text-white text-2xl font-medium pointer-events-none">
